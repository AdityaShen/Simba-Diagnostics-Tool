/**
 * @file videoPlaybackService.js
 * @description Handles video playback, decoding, and stream management for Simba client.
...
if (typeof setLoggerFunc === "function") {
    setLoggerFunc(
        () => {},
        (message) => appendLog(message, true),
    );
}
...
function onMseVideoCanPlay() {
    if (
        globalState.decoderType === C.DECODER_TYPES.MSE &&
        globalState.isRunning &&
        elements.videoElement &&
        typeof VideoConverter === "function"
    ) {
...
/**
 * Updates global state and UI for new video resolution info.
...
export function handleVideoInfo(width, height) {
    ...
    if (elements.streamArea) {
        elements.streamArea.style.aspectRatio =
            width > 0 && height > 0 ? `${width} / ${height}` : "9 / 16";
    }
    ...
    const targetCanvas =
        globalState.decoderType === C.DECODER_TYPES.WEBCODECS
            ? elements.webcodecCanvas
            : globalState.decoderType === C.DECODER_TYPES.BROADWAY
              ? globalState.broadwayPlayer
...
/**
 * Initializes video playback for the selected decoder type (MSE, Broadway, WebCodecs).
 */
export function initializeVideoPlayback() {
    if (
        globalState.broadwayPlayer &&
        globalState.broadwayPlayer.canvas &&
        globalState.broadwayPlayer.canvas.parentElement
    ) {
        globalState.broadwayPlayer.canvas.parentElement.removeChild(
            globalState.broadwayPlayer.canvas,
        );
    }
    ...
    if (globalState.decoderType === C.DECODER_TYPES.MSE) {
        ...
        globalState.converter = new VideoConverter(
            elements.videoElement,
            fps,
            C.DEFAULT_FRAMES_PER_FRAGMENT,
        );
        ...
        if (typeof Player !== "function") {
            if (Player && typeof Player.Player === "function")
                ...
                appendLog("Broadway Player class not available.", true);
                return;
            }
        }
        globalState.broadwayPlayer = new BroadwayPlayerInstance({
            useWorker: true,
            workerFile: "vendor/node_modules/broadway-player/Player/Decoder.js",
            webgl: "auto",
            reuseMemory: true,
        });
        if (!globalState.broadwayPlayer || !globalState.broadwayPlayer.canvas) {
            appendLog("Broadway: Player or player.canvas is null!", true);
            globalState.broadwayPlayer = null;
            return;
        }
        if (
            elements.broadwayCanvas &&
            elements.broadwayCanvas !== globalState.broadwayPlayer.canvas
        ) {
        ...
        if (!window.VideoDecoder) {
            appendLog("VideoDecoder API not supported.", true);
            return;
        }
        if (elements.webcodecCanvas) {
            elements.webcodecCanvas.style.display = "block";
            globalState.webCodecCanvasCtx = elements.webcodecCanvas.getContext("2d", {
                alpha: false,
            });
        }
        globalState.webCodecsVideoDecoder = new VideoDecoder({
            output: (videoFrame) => {
                if (
                    globalState.webCodecCanvasCtx &&
                    videoFrame &&
                    elements.webcodecCanvas.width > 0 &&
                    elements.webcodecCanvas.height > 0
                ) {
                    globalState.webCodecCanvasCtx.drawImage(
                        videoFrame,
                        0,
                        0,
...
/**
 * Stops video playback and cleans up decoder resources for all decoder types.
 */
export function stopVideoPlayback() {
    if (globalState.decoderType === C.DECODER_TYPES.MSE) {
        if (globalState.converter) {
            try {
                ...
                appendLog(`Error stopping MSE converter: ${e.message}`, true);
            }
        ...
        if (
            globalState.webCodecsVideoDecoder &&
            globalState.webCodecsVideoDecoder.state !== "closed"
        ) {
            globalState.webCodecsVideoDecoder.close();
        }
        globalState.webCodecsVideoDecoder = null;
    ...
    handleVideoInfo(0, 0);
    if (elements.videoElement && globalState.decoderType === C.DECODER_TYPES.MSE)
        elements.videoElement.style.display = "block";
    else if (elements.videoElement) elements.videoElement.style.display = "none";

    if (elements.broadwayCanvas && globalState.decoderType !== C.DECODER_TYPES.BROADWAY)
        elements.broadwayCanvas.style.display = "none";
    if (elements.webcodecCanvas && globalState.decoderType !== C.DECODER_TYPES.WEBCODECS)
        elements.webcodecCanvas.style.display = "none";
}
...
function isIFrame(frameData) {
    ...
    return (
        offset > 0 && frameData.length > offset && (frameData[offset] & 0x1f) === C.NALU_TYPE_IDR
    );
}

function cleanSourceBuffer() {
    if (
        !globalState.sourceBufferInternal ||
        globalState.sourceBufferInternal.updating ||
        globalState.removeStart < 0 ||
        globalState.removeEnd <= globalState.removeStart
    ) {
    ...
    try {
        ...
        globalState.sourceBufferInternal.addEventListener("updateend", cleanSourceBuffer, {
            once: true,
        });
    } catch (e) {
...
function checkForIFrameAndCleanBuffer(frameData) {
    ...
    if (end !== 0 && start < end) {
        if (globalState.removeEnd !== -1)
            globalState.removeEnd = Math.max(globalState.removeEnd, end);
        else {
            globalState.removeStart = start;
            globalState.removeEnd = end;
        }
        if (globalState.sourceBufferInternal) {
            globalState.sourceBufferInternal.addEventListener("updateend", cleanSourceBuffer, {
                once: true,
...
function annexBToLengthPrefixed(annexBBuffer) {
    ...
    while (offset < len) {
        ...
        if (
            offset + 3 <= len &&
            annexBBuffer[offset] === 0 &&
            annexBBuffer[offset + 1] === 0 &&
            annexBBuffer[offset + 2] === 1
        ) {
            startCodeLength = 3;
        } else if (
            offset + 4 <= len &&
            annexBBuffer[offset] === 0 &&
            annexBBuffer[offset + 1] === 0 &&
            annexBBuffer[offset + 2] === 0 &&
            annexBBuffer[offset + 3] === 1
        ) {
            startCodeLength = 4;
        }
        ...
        if (startCodeLength === 0) {
            ...
            if (len - offset > 0) {
                ...
                new DataView(lengthBuffer.buffer).setUint32(0, remaining.byteLength, false);
        ...
        while (searchOffset + 3 <= len) {
            if (
                searchOffset + 4 <= len &&
                annexBBuffer[searchOffset] === 0 &&
                annexBBuffer[searchOffset + 1] === 0 &&
                annexBBuffer[searchOffset + 2] === 0 &&
                annexBBuffer[searchOffset + 3] === 1
            ) {
...
/**
 * Handles incoming video data and decodes it using the selected decoder.
 * @param {ArrayBuffer} payloadArrayBuffer - Raw video data.
 * @param {number|null} timestamp - Frame timestamp (WebCodecs only).
 * @param {string|null} frameType - Frame type ('key' or 'delta', WebCodecs only).
 */
export function handleVideoData(payloadArrayBuffer, timestamp = null, frameType = null) {
    ...
    if (globalState.decoderType === C.DECODER_TYPES.MSE) {
        ...
        globalState.inputBytes.push({
            timestamp: Date.now(),
            bytes: payloadArrayBuffer.byteLength,
        });
        ...
        if (!globalState.webCodecsVideoDecoder || timestamp === null || frameType === null) {
            if (!globalState.webCodecsVideoDecoder)
                appendLog("WebCodecs Video: Decoder not ready.", true);
...
/**
 * Configures the WebCodecs video decoder with SPS/AVCC data.
 * @param {number} spsProfile - SPS profile.
 * @param {number} spsCompat - SPS compatibility.
 * @param {number} spsLevel - SPS level.
 * @param {ArrayBuffer} avccBuffer - AVCC buffer.
 */
export function configureWebCodecsVideoDecoder(spsProfile, spsCompat, spsLevel, avccBuffer) {
    if (
        ...
        if (globalState.webCodecsVideoDecoder.state === "configured") {
            ...
            try {
                globalState.webCodecsVideoDecoder.reset();
            } catch (e) {
                appendLog(`Error resetting decoder: ${e.message}`, true);
            }
        }
        try {
            ...
            appendLog(
                `Configuring WebCodecs VideoDecoder with: codec: ${codecString}, description length: ${avccBuffer.byteLength}`,
            );
...
function calculateMomentumStats() {
    ...
    globalState.inputBytes.push({
        timestamp,
        bytes:
            globalState.inputBytes.length > 0
                ? globalState.inputBytes[globalState.inputBytes.length - 1].bytes
                : 0,
    });
    if (globalState.videoStats.length > 10) {
        globalState.videoStats.shift();
        globalState.inputBytes.shift();
    }
    ...
    if (globalState.videoStats.length > 1) {
        ...
        globalState.momentumQualityStats = {
            decodedFrames,
            droppedFrames,
...
/**
 * Checks for bad video playback state and attempts recovery if needed.
 */
export function checkForBadState() {
    ...
    if (
        !globalState.isRunning ||
        !globalState.converter ||
        !elements.videoElement ||
        elements.videoElement.paused ||
        typeof VideoConverter !== "function"
    )
        return;
    ...
    if (
        globalState.momentumQualityStats &&
        globalState.momentumQualityStats.decodedFrames === 0 &&
        globalState.momentumQualityStats.inputFrames > 0
    ) {
        if (globalState.noDecodedFramesSince === -1) globalState.noDecodedFramesSince = now;
        else if (now - globalState.noDecodedFramesSince > C.MAX_TIME_TO_RECOVER)
            hasReasonToJump = true;
    } else globalState.noDecodedFramesSince = -1;

    if (currentTime === globalState.lastVideoTime && globalState.currentTimeNotChangedSince === -1)
        globalState.currentTimeNotChangedSince = now;
    ...
    if (elements.videoElement.buffered && elements.videoElement.buffered.length > 0) {
        ...
        if (buffered > MAX_BUFFER_JUMP) {
            ...
            else if (now - globalState.bigBufferSince > C.MAX_TIME_TO_RECOVER)
                hasReasonToJump = true;
        ...
        if (buffered < C.MAX_AHEAD) {
            ...
            else if (now - globalState.aheadOfBufferSince > C.MAX_TIME_TO_RECOVER)
                hasReasonToJump = true;
        } else globalState.aheadOfBufferSince = -1;
        if (
            globalState.currentTimeNotChangedSince !== -1 &&
            now - globalState.currentTimeNotChangedSince > C.MAX_TIME_TO_RECOVER
        )
            hasReasonToJump = true;
        if (!hasReasonToJump) return;
        if (globalState.seekingSince !== -1 && now - globalState.seekingSince < C.MAX_SEEK_WAIT_MS)
            return;
        const onSeekEnd = () => {
            globalState.seekingSince = -1;
            elements.videoElement.removeEventListener("seeked", onSeekEnd);
            elements.videoElement.play().catch((e) => {
                appendLog(`Video play error after seek: ${e.message}`, true);
            });
        };
        if (globalState.seekingSince !== -1)
            appendLog("Seek already in progress, but attempting again due to bad state.", true);
...
/**
 * Updates the video resolution in the stream and UI if changed.

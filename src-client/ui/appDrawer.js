/**
 * @file appDrawer.js
 * @description UI logic for the app drawer, pagination, and app launching in Simba client.
 * @author Aditya Shen
...
function showPage(pageNumber) {
    ...
    if (targetPage > globalState.totalPages && globalState.totalPages > 0)
        targetPage = globalState.totalPages;
...
/**
 * Renders the app drawer grid and pagination controls.
 * @param {Array} apps - List of app objects to display.
 */
export function renderAppDrawer(apps) {
    ...
    if (globalState.allApps.length === 0) {
        ...
        for (let i = 0; i < globalState.totalPages; i++) {
            ...
            const pageApps = globalState.allApps.slice(
                i * globalState.appsPerPage,
                (i + 1) * globalState.appsPerPage,
            );
...
/**
 * Initializes all event listeners and UI logic for the app drawer.
 */

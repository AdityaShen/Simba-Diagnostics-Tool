/**
 * @file taskbarControls.js
 * @description UI logic for taskbar, quick settings, audio, and navigation controls in Simba client.
 * @author Aditya Shen
...
import {
    HIDE_TASKBAR_TIMEOUT_MS,
    DOUBLE_CLICK_THRESHOLD_MS,
    CONTROL_MSG_TYPE_SET_SCREEN_POWER_MODE_CLIENT,
    SCREEN_POWER_MODE_OFF_CLIENT,
...
/**
 * Updates the clock display in the taskbar UI.
 */
...
/**
 * Updates the Wi-Fi indicator and toggle button in the taskbar UI.
 */
...
function updatePinToggleIcon() {
    if (elements.pinToggleButton) {
        ...
        elements.pinToggleButton.setAttribute(
            "aria-label",
            globalState.isTaskbarPinned ? "Unpin Taskbar" : "Pin Taskbar",
        );
    }
}

/**
 * Updates the speaker icon in the taskbar and audio panel based on volume.
 * @param {number} volume - The current volume value.
 */
export function updateSpeakerIconFromVolume(volume) {
    ...
    if (elements.speakerButton) {
        ...
        elements.speakerButton.setAttribute(
            "aria-label",
            isMuted ? "Audio Muted" : "Audio Settings",
        );
    }
...
/**
 * Updates the background of a slider element to reflect its value.
...
/**
 * Updates the battery level indicator in the taskbar UI.
 * @param {number|string} level - The battery level value.
 */
...
function handlePinToggle(isDoubleClick = false) {
    if (isDoubleClick) {
        ...
        if (elements.taskbar)
            elements.taskbar.classList.toggle("pinned", globalState.isTaskbarPinned);
        updatePinToggleIcon();
    }
...
function turnScreenOff() {
    if (!globalState.isRunning) {
        appendLog("Stream not active.", true);
        return;
    }
    if (!globalState.controlEnabledAtStart) {
        appendLog("Control not enabled.", true);
        return;
    }
...
/**
 * Opens a panel (audio, quick settings, app drawer) in the UI and updates state.
...
/**
 * Closes the currently active panel in the UI and updates state.
 */
export function closeActivePanel() {
    if (globalState.activePanel) {
        const panelToCloseId = globalState.activePanel;
        const panelToClose =
            document.getElementById(panelToCloseId) ||
            (elements[panelToCloseId] && elements[panelToCloseId].classList?.contains("app-drawer")
                ? elements[panelToCloseId]
                : null);
...
/**
 * Initializes all event listeners and UI logic for the taskbar and related controls.
 */
export function initTaskbarControls() {
    ...
    if (elements.mediaVolumeSlider) {
        elements.mediaVolumeSlider.addEventListener("input", () => {
            ...
            if (now - globalState.lastVolumeSendTime > VOLUME_THROTTLE_MS) {
                ...
                globalState.volumeChangeTimeout = setTimeout(
                    () => {
                        if (globalState.pendingVolumeValue !== null)
                            sendVolumeUpdateInternal(globalState.pendingVolumeValue);
        ...
        const sendFinalVolume = () => {
            if (globalState.volumeChangeTimeout) {
                clearTimeout(globalState.volumeChangeTimeout);
                globalState.volumeChangeTimeout = null;
            }
    ...
    document.addEventListener("click", (e) => {
        ...
        if (globalState.activePanel) {
            ...
            if (globalState.activePanel === "appDrawer") {
                if (
                    elements.appDrawer?.contains(target) ||
                    target === elements.appDrawerButton ||
                    elements.appDrawerButton?.contains(target)
                ) {
                ...
                if (globalState.activePanel === "audioPanel")
                    triggerElement = elements.speakerButton;
                else if (globalState.activePanel === "quickSettingsPanel")
                    triggerElement = elements.quickSettingsTrigger;
    ...
    updateSpeakerIconFromVolume(
        elements.mediaVolumeSlider ? parseInt(elements.mediaVolumeSlider.value, 10) : 0,
    );
    updateSliderBackground(elements.mediaVolumeSlider);
}

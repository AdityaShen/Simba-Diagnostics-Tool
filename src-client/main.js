...
document.addEventListener("DOMContentLoaded", () => {
     ...
     */
            if (adbShellModal) {
                ...
                // Start shell session
                sendWebSocketMessage({ action: "startAdbShell", deviceId: globalState.selectedDeviceId });
            }
        }
    /**
     ...
     */
            ...
            adbShellSendBtn.addEventListener("click", () => {
                ...
                if (cmd && adbShellSessionActive) {
                    sendWebSocketMessage({ action: "adbShellInput", deviceId: globalState.selectedDeviceId, input: cmd });
                    adbShellInput.value = "";
                }
            });
            /**
            ...
            globalState.ws.addEventListener("message", (event) => {
                try {
                    ...
                    if (message.type === "adbShellClosed") {
                        if (adbShellOutput) adbShellOutput.innerHTML += "<br><span style='color:#ffa07a;'>[Shell session closed]</span>";
                        adbShellSessionActive = false;
                    }
     ...
     */
        if (harTraceUrlModal) {
            ...
            setTimeout(() => {
                harTraceUrlInput.setSelectionRange(harTraceUrlInput.value.length, harTraceUrlInput.value.length);
            }, 0);
        }
    }
 ...
 */
            ...
            try {
                ...
                if (message.type === "harTraceResponse" && elements.harTraceStatus) {
                    if (message.success) {
                        ...
                        elements.harTraceStatus.innerHTML = "HAR trace complete.<br><button id=\"saveHarFileBtn\">Save As...</button>";
                        const saveBtn = document.getElementById("saveHarFileBtn");
                        if (saveBtn && window && window.require) {
                            saveBtn.addEventListener("click", () => {
                                try {
                                    const { ipcRenderer } = window.require("electron");
                                    ipcRenderer.invoke("save-har-file", message.file).then((result) => {
                                        if (result.success) {
                                    ...
                                    elements.harTraceStatus.textContent = "HAR trace complete, but save dialog failed.";
                                }
                            });
                        }
                    } else {
     ...
     */
        startBtn.addEventListener("click", () => {
            ...
            recordedChunks = [];
            try {
                mediaRecorder = new window.MediaRecorder(stream, { mimeType: "video/webm; codecs=vp9" });
            } catch (e) {
                try {

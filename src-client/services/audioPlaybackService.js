/**
 * @file audioPlaybackService.js
 * @description Handles audio playback, decoding, and stream management for Simba client.
 * @author Aditya Shen
...
/**
 * Sets up the audio player and decoder for AAC streams using WebCodecs.
 * @param {number} codecId - Codec ID (should be AAC).
 * @param {Object} metadata - Audio stream metadata.
 * @param {ArrayBuffer|null} audioConfigData - Optional codec config data.
 */
export function setupAudioPlayer(codecId, metadata, audioConfigData = null) {
    ...
    try {
        if (!globalState.audioContext || globalState.audioContext.state === "closed") {
            globalState.audioContext = new AudioContext({
                sampleRate: metadata?.sampleRate || 48000,
            });
        }

        if (
            globalState.webCodecsAudioDecoder &&
            globalState.webCodecsAudioDecoder.state !== "closed"
        ) {
        ...
        globalState.webCodecsAudioDecoder = new AudioDecoder({
            output: (audioData) => {
                try {
                    ...
                    const buffer = globalState.audioContext.createBuffer(
                        numberOfChannels,
                        audioData.numberOfFrames,
                        sampleRate,
                    );
                    const isInterleaved =
                        audioData.format === "f32" || audioData.format === "f32-interleaved";
                    if (isInterleaved) {
                    ...
                    const videoTime =
                        globalState.decoderType === "mse" && elements.videoElement
                            ? elements.videoElement.currentTime
                            : 0;
                    ...
                    if (globalState.nextAudioTime < currentTime)
                        globalState.nextAudioTime = currentTime;
...
/**
 * Handles incoming audio data and decodes it using WebCodecs.
...
export function handleAudioData(arrayBuffer, timestamp) {
    if (
        ...
        globalState.audioCodecId !== C.CODEC_IDS.AAC ||
        arrayBuffer.byteLength === 0
    )
        return;
    try {
        globalState.webCodecsAudioDecoder.decode(
...
/**
 * Closes the audio decoder and context, cleaning up resources.
 */
export function closeAudio() {
    if (globalState.webCodecsAudioDecoder) {
        if (globalState.webCodecsAudioDecoder.state !== "closed")
            globalState.webCodecsAudioDecoder.close();
        globalState.webCodecsAudioDecoder = null;
    }

/**
 * @file inputService.js
 * @description Handles mouse, touch, and input events for device control in Simba client.
 * @author Aditya Shen
...
function getScaledCoordinates(event) {
    ...
    if (
        ...
        !globalState.deviceWidth ||
        !globalState.deviceHeight
    ) {
        return null;
    }
...
function sendMouseEvent(action, buttons, x, y) {
    if (!globalState.deviceWidth || !globalState.deviceHeight || !globalState.controlEnabledAtStart)
        return;
...
function handleMouseDown(event) {
    ...
    if (globalState.decoderType === C.DECODER_TYPES.BROADWAY) {
        activeRenderingElement = globalState.broadwayPlayer
    ...
    if (
        !activeRenderingElement ||
        !event.target ||
        (event.target !== activeRenderingElement && !activeRenderingElement.contains(event.target))
    ) {
    ...
    switch (event.button) {
        case 0:
            buttonFlag = C.AMOTION_EVENT_BUTTON_PRIMARY;
            break;
        case 1:
            buttonFlag = C.AMOTION_EVENT_BUTTON_TERTIARY;
            break;
        default:
            return;
    }
    ...
    if (coords) {
        ...
        sendMouseEvent(
            ...
            globalState.currentMouseButtons,
            coords.x,
            coords.y,
        );
    }
...
function handleMouseUp(event) {
    ...
    switch (event.button) {
        ...
        case 1:
            ...
            break;
        case 2:
            buttonFlag = C.AMOTION_EVENT_BUTTON_SECONDARY;
            break;
        default:
            return;
    }
...
function handleMouseLeave(event) {
    ...
    sendMouseEvent(
        C.AMOTION_EVENT_ACTION_UP,
        globalState.currentMouseButtons,
        globalState.lastMousePosition.x,
        globalState.lastMousePosition.y,
    );
    globalState.isMouseDown = false;
    globalState.currentMouseButtons = 0;
}
...
function handleWheelEvent(event) {
    ...
    const dataView = new DataView(buffer);
    let offset = 0;
    dataView.setUint8(offset, C.CONTROL_MSG_TYPE_SCROLL_CLIENT);
    offset += 1;
    dataView.setInt32(offset, coords.x, false);
    offset += 4;
    dataView.setInt32(offset, coords.y, false);
    offset += 4;
    dataView.setUint16(offset, globalState.deviceWidth, false);
    offset += 2;
    dataView.setUint16(offset, globalState.deviceHeight, false);
    offset += 2;
...
/**
 * Initializes all event listeners for mouse, touch, and input events on the video/canvas area.
 */

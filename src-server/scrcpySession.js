/**
 * @file scrcpySession.js
 * @description Manages scrcpy streaming sessions, TCP server, socket handling, and session lifecycle for device control and media streaming.
 * @module scrcpySession
 * @author Simba Team
 * @copyright Simba (c) 2025
 * @license MIT
 *
 * This module provides session management, streaming, and cleanup for scrcpy-based device control and media streaming.
 */
...
/**
 * BitReader for parsing H.264 SPS and other bitstream data.
 */
class BitReader {
    constructor(buffer) {
        this.buffer = buffer;
...
/**
 * Parses H.264 SPS NALU to extract video resolution and profile info.
 * @param {Buffer} naluBufferWithStartCode - NALU buffer with start code.
 * @returns {Object|null} Parsed SPS info or null on error.
 */
function parseSPS(naluBufferWithStartCode) {
    if (!naluBufferWithStartCode || naluBufferWithStartCode.length < 1) return null;
    let offset = 0;
    if (
        naluBufferWithStartCode.length >= 4 &&
        naluBufferWithStartCode[0] === 0 &&
        naluBufferWithStartCode[1] === 0 &&
        naluBufferWithStartCode[2] === 0 &&
        naluBufferWithStartCode[3] === 1
    ) {
        offset = 4;
    } else if (
        naluBufferWithStartCode.length >= 3 &&
        naluBufferWithStartCode[0] === 0 &&
        naluBufferWithStartCode[1] === 0 &&
        naluBufferWithStartCode[2] === 1
    ) {
        offset = 3;
    }

    const rbspBuffer = naluBufferWithStartCode.subarray(offset);
    if (rbspBuffer.length < 4) return null;
    ...
    reader.readBits(8); // constraint_set_flags (unused)
    reader.readBits(8); // level_idc (unused)
        reader.readUE();
        ...
        if (
            pic_width_in_mbs_minus1 === null ||
            pic_height_in_map_units_minus1 === null ||
            frame_mbs_only_flag === null
        )
            return null;
        ...
        if (frame_cropping_flag) {
            ...
            if (
                frame_crop_left_offset === null ||
                frame_crop_right_offset === null ||
                frame_crop_top_offset === null ||
        ...
        return {
            width,
            height,
            profile_idc: profile_idc_val,
            profile_compatibility: profile_compatibility_val,
            level_idc: level_idc_val,
        };
    } catch (e) {
        log(C.LogLevel.WARN, `Error parsing SPS: ${e.message}`);
        return null;
    }
}

/**
 * Parses AAC AudioSpecificConfig to extract profile, sample rate, and channel config.
 * @param {Buffer} buffer - AudioSpecificConfig buffer.
 * @returns {Object|null} Parsed audio config or null on error.
 */
function parseAudioSpecificConfig(buffer) {
    ...
    return {
        profile: C.PROFILE_MAP[objectType],
        sampleRateIndex,
        sampleRate,
        channelConfig,
        rawASC: buffer,
...
/**
 * Handles initial device name data from socket.
 */
function _handleAwaitingInitialData(socket, dynBuffer, session, client) {
    if (!session.deviceNameReceived) {
        if (dynBuffer.length >= C.DEVICE_NAME_LENGTH) {
            const deviceName = dynBuffer.buffer
                .subarray(0, C.DEVICE_NAME_LENGTH)
                .toString("utf8")
                .split("\0")[0];
...
/**
 * Handles metadata identification for video/audio/control sockets.
 */
function _handleAwaitingMetadata(socket, dynBuffer, session, client) {
    ...
    if (!session.videoSocket && session.expectedSockets.includes("video")) {
        if (dynBuffer.length >= C.VIDEO_METADATA_LENGTH_H264) {
            ...
            if (potentialCodecId === C.CODEC_IDS.H264) {
                ...
                log(
                    C.LogLevel.INFO,
                    `[Session ${session.scid}] Identified Video socket (${width}x${height}) for ${session.decoderType}`,
                );
                ...
                client.ws.send(
                    JSON.stringify({
                        type: C.MESSAGE_TYPES.VIDEO_INFO,
                        codecId: potentialCodecId,
                        width,
                        height,
                    }),
                );
                dynBuffer.buffer.copy(
                    dynBuffer.buffer,
                    0,
                    C.VIDEO_METADATA_LENGTH_H264,
                    dynBuffer.length,
                );
                dynBuffer.length -= C.VIDEO_METADATA_LENGTH_H264;
    ...
    if (!identifiedThisPass && !session.audioSocket && session.expectedSockets.includes("audio")) {
        if (dynBuffer.length >= C.AUDIO_METADATA_LENGTH_AAC) {
            ...
            if (potentialCodecId === C.CODEC_IDS.AAC) {
                ...
                session.unidentifiedSockets?.delete(socket.remoteId);
                client.ws.send(
                    JSON.stringify({ type: C.MESSAGE_TYPES.AUDIO_INFO, codecId: potentialCodecId }),
                );
                dynBuffer.buffer.copy(
                    dynBuffer.buffer,
                    0,
                    C.AUDIO_METADATA_LENGTH_AAC,
                    dynBuffer.length,
                );
                dynBuffer.length -= C.AUDIO_METADATA_LENGTH_AAC;
                socket.state = "STREAMING";
                checkAndSendStreamingStarted(session, client);
            }
        }
    ...
    if (
        ...
        session.expectedSockets.length === 1 &&
        session.expectedSockets[0] === "control" &&
        socket.didHandleDeviceName
    ) {
        log(
            C.LogLevel.INFO,
            `[Session ${session.scid}] Identified Control socket (only expected stream)`,
        );
        ...
        session.unidentifiedSockets?.delete(socket.remoteId);
        socket.state = "STREAMING";
        const worker = new Worker(path.resolve(__dirname, "controlWorker.js"), {
            workerData: {
                scid: session.scid,
                clientId: session.clientId,
                CURRENT_LOG_LEVEL: C.CURRENT_LOG_LEVEL,
            },
        });
        ...
        worker.on("message", (msg) => {
            if (msg.type === "writeToSocket") {
                ...
                if (currentSession?.controlSocket && !currentSession.controlSocket.destroyed) {
                    try {
                        currentSession.controlSocket.write(
                            Buffer.from(msg.data.data ? msg.data.data : msg.data),
                        );
                    } catch (e) {
        ...
        worker.on("error", (err) => {
            log(C.LogLevel.ERROR, `[Worker ${session.scid}] Error: ${err.message}`);
            workers.delete(session.scid);
        });
        worker.on("exit", (code) => {
            log(C.LogLevel.INFO, `[Worker ${session.scid}] Exited with code ${code}`);
            workers.delete(session.scid);
        });
        checkAndSendStreamingStarted(session, client);
    }
    if (identifiedThisPass) {
        ...
        if (
            !session.controlSocket &&
            session.expectedSockets.includes("control") &&
            session.unidentifiedSockets?.has(socket.remoteId)
        )
            return false;
...
/**
 * Extracts NALU payload by removing start code.
 * @param {Buffer} naluWithStartCode - NALU buffer with start code.
 * @returns {Buffer} NALU payload.
 */
function extractNaluPayload(naluWithStartCode) {
    ...
    if (naluWithStartCode[0] === 0 && naluWithStartCode[1] === 0) {
        ...
        if (
            naluWithStartCode.length > 3 &&
            naluWithStartCode[2] === 0 &&
            naluWithStartCode[3] === 1
        )
            return naluWithStartCode.subarray(4);
    }
    return naluWithStartCode;
}

/**
 * Extracts a single NALU of the specified type from buffer.
 * @param {Buffer} buffer - Buffer containing NALUs.
 * @param {number} naluTypeToFind - NALU type to find.
 * @returns {Buffer|null} Found NALU or null.
 */
...
/**
 * Creates AVCC box from SPS and PPS NALUs for H.264 WebCodecs.
 * @param {Buffer} spsNaluWithStartCode - SPS NALU buffer.
 * @param {Buffer} ppsNaluWithStartCode - PPS NALU buffer.
 * @returns {Buffer|null} AVCC box or null.
 */
function createAVCC(spsNaluWithStartCode, ppsNaluWithStartCode) {
    ...
    if (sps.length === 0 || pps.length === 0) {
        log(
            C.LogLevel.WARN,
            `createAVCC: SPS or PPS data is empty after attempting to strip start codes. Original SPS len: ${spsNaluWithStartCode.length}, PPS len: ${ppsNaluWithStartCode.length}`,
        );
        return null;
    }
    if (sps.length < 4) {
        log(
            C.LogLevel.WARN,
            `createAVCC: SPS data too short after stripping. Length: ${sps.length}`,
        );
        return null;
    }

    const avccBox = Buffer.alloc(1 + 3 + 1 + 1 + 2 + sps.length + 1 + 2 + pps.length);
    ...
    avccBox.writeUInt16BE(sps.length, offset);
    offset += 2;
    sps.copy(avccBox, offset);
    offset += sps.length;

    avccBox.writeUInt8(1, offset++);
    avccBox.writeUInt16BE(pps.length, offset);
    offset += 2;
    pps.copy(avccBox, offset);
    offset += pps.length;
...
/**
 * Processes a video stream packet and sends to client.
 */
function _processVideoStreamPacket(socket, dynBuffer, session, client) {
    if (dynBuffer.length >= C.PACKET_HEADER_LENGTH) {
        ...
        if (packetSize > 10 * 1024 * 1024 || packetSize < 0) {
            log(C.LogLevel.ERROR, `[TCP Video ${socket.scid}] Invalid packet size: ${packetSize}`);
        ...
        if (dynBuffer.length >= totalPacketLength) {
            ...
            if (session.decoderType === "webcodecs") {
                ...
                if (configFlagInt) {
                    log(
                        C.LogLevel.DEBUG,
                        `[TCP Video ${socket.scid}] Received config packet for WebCodecs. Payload length: ${payload.length}`,
                    );
                    ...
                    if (spsNaluWithStartCode && ppsNaluWithStartCode) {
                        log(
                            C.LogLevel.DEBUG,
                            `[TCP Video ${socket.scid}] Extracted SPS (len ${spsNaluWithStartCode.length}) and PPS (len ${ppsNaluWithStartCode.length}) from config payload.`,
                        );
                        ...
                        if (avcc && spsData) {
                            ...
                            log(
                                C.LogLevel.INFO,
                                `[TCP Video ${socket.scid}] Sent AVCC config (len ${avcc.length}) with SPS info to client.`,
                            );
                        } else {
                            log(
                                C.LogLevel.WARN,
                                `[TCP Video ${socket.scid}] Failed to create AVCC or parse SPS from extracted NALUs. AVCC: ${!!avcc}, SPSData: ${!!spsData}`,
                            );
                        }
                        if (spsData) {
                            const newWidth = spsData.width,
                                newHeight = spsData.height;
                            if (
                                ...
                                log(
                                    C.LogLevel.INFO,
                                    `[TCP Video ${socket.scid}] Resolution change from config: ${newWidth}x${newHeight}`,
                                );
                                session.currentWidth = newWidth;
                                session.currentHeight = newHeight;
                                if (client && client.ws?.readyState === WebSocket.OPEN)
                                    client.ws.send(
                                        JSON.stringify({
                                            type: C.MESSAGE_TYPES.RESOLUTION_CHANGE,
                                            width: newWidth,
                                            height: newHeight,
                                        }),
                                    );
                        ...
                        log(
                            C.LogLevel.WARN,
                            `[TCP Video ${socket.scid}] Could not extract SPS/PPS from config packet payload. SPS: ${!!spsNaluWithStartCode}, PPS: ${!!ppsNaluWithStartCode}. Config Payload length: ${payload.length}`,
                        );
                    }
                } else {
                    packetType = keyFrameFlagInt
                        ? C.BINARY_PACKET_TYPES.WC_VIDEO_KEY_FRAME_H264
                        : C.BINARY_PACKET_TYPES.WC_VIDEO_DELTA_FRAME_H264;
                ...
                typeBuffer.writeUInt8(C.BINARY_PACKET_TYPES.LEGACY_VIDEO_H264, 0);
...
/**
 * Processes an audio stream packet and sends to client.
 */
function _processAudioStreamPacket(socket, dynBuffer, session, client) {
    if (dynBuffer.length >= C.PACKET_HEADER_LENGTH) {
        ...
        if (packetSize > 10 * 1024 * 1024 || packetSize < 0) {
            log(C.LogLevel.ERROR, `[TCP Audio ${socket.scid}] Invalid packet size: ${packetSize}`);
            socket.state = "UNKNOWN";
            socket.destroy();
            return false;
        }
            ...
            if (configFlagInt) {
                try {
                    ...
                    if (session.audioMetadata && session.audioMetadata.rawASC) {
                        ...
                        client.ws.send(Buffer.concat([header, session.audioMetadata.rawASC]), {
                            binary: true,
                        });
                        log(
                            C.LogLevel.DEBUG,
                            `[TCP Audio ${socket.scid}] Sent ASC config to client.`,
                        );
                    } else {
                        log(
                            C.LogLevel.ERROR,
                            `[TCP Audio ${socket.scid}] Failed to get raw ASC from audio config.`,
                        );
                    }
                } catch (e) {
                    log(
                        C.LogLevel.ERROR,
                        `[TCP Audio ${socket.scid}] Failed to parse audio config: ${e.message}`,
                    );
                    socket.destroy();
                    return false;
                }
...
/**
 * Handles streaming data for video, audio, or control socket.
 */
...
/**
 * Processes a single socket's state and data.
 */
    ...
    while (keepProcessing && !socket.destroyed && socket.state !== "UNKNOWN") {
        ...
        switch (socket.state) {
            ...
            case "AWAITING_METADATA":
                ...
                break;
            case "STREAMING":
                if (_handleStreamingData(socket, dynBuffer, session, client)) keepProcessing = true;
                break;
            default:
                socket.state = "UNKNOWN";
                log(
                    C.LogLevel.ERROR,
                    `[TCP] Socket ${socket.scid} from ${socket.remoteId} invalid state: ${socket.state}.`,
                );
                break;
        }
    }
...
/**
 * Creates a TCP server for scrcpy session and handles socket connections.
 * @param {string} scid - Session ID.
 * @param {Map} wsClientsRef - Reference to WebSocket clients.
...
function createTcpServer(scid, wsClientsRef) {
    const server = net.createServer((socket) => {
        ...
        log(
            C.LogLevel.DEBUG,
            `[TCP] Socket connected for ${scid} from ${remoteId}. Total: ${session.socketsConnected}/${session.expectedSockets.length}`,
        );
        socket.on("data", (data) => {
            ...
            const dynBuf = socket.dynamicBuffer;
            const requiredLength = dynBuf.length + data.length;
            if (requiredLength > dynBuf.buffer.length) {
                const newSize = Math.max(dynBuf.buffer.length * 2, requiredLength + 1024);
                try {
                    const newBuffer = Buffer.allocUnsafe(newSize);
                    dynBuf.buffer.copy(newBuffer, 0, 0, dynBuf.length);
                    dynBuf.buffer = newBuffer;
                } catch (e) {
                    log(C.LogLevel.ERROR, `[TCP] Buffer alloc fail ${socket.scid}: ${e.message}`);
                    socket.destroy();
                    cleanupSession(socket.scid, wsClientsRef);
                    return;
                }
            }
            data.copy(dynBuf.buffer, dynBuf.length);
            dynBuf.length += data.length;
            processSingleSocket(socket, currentClient, currentSession);
            if (dynBuf.length === 0 && dynBuf.buffer.length > 1024 * 512)
                try {
                    dynBuf.buffer = Buffer.alloc(1024 * 512);
                } catch {} // No catch parameter needed
        });
        socket.on("end", () => {
            log(C.LogLevel.DEBUG, `[TCP] Socket ended ${scid} from ${remoteId}`);
            clearSocketReference(scid, socket, wsClientsRef);
            sessions.get(scid)?.unidentifiedSockets?.delete(remoteId);
        });
        socket.on("close", (hadError) => {
            log(
                C.LogLevel.DEBUG,
                `[TCP] Socket closed ${scid} from ${remoteId}. Error: ${hadError}`,
            );
            clearSocketReference(scid, socket, wsClientsRef);
            sessions.get(scid)?.unidentifiedSockets?.delete(remoteId);
        });
        socket.on("error", (err) => {
            log(C.LogLevel.ERROR, `[TCP] Socket error ${scid} from ${remoteId}: ${err.message}`);
            clearSocketReference(scid, socket, wsClientsRef);
            sessions.get(scid)?.unidentifiedSockets?.delete(remoteId);
            socket.destroy();
        });
    ...
    server.on("error", (err) => {
        log(C.LogLevel.ERROR, `[TCP] Server error for ${scid}: ${err.message}`);
...
/**
 * Clears socket references and triggers cleanup if all sockets closed.
 */
...
/**
 * Checks if all streams are ready and notifies client that streaming started.
 */
async function checkAndSendStreamingStarted(session, client) {
    if (
        ...
        session.streamingStartedNotified
    )
        return;
    ...
    if (videoReady && audioReady && controlReady) {
        client.ws.send(
            JSON.stringify({ type: C.MESSAGE_TYPES.STATUS, message: "Streaming started" }),
        );
        session.streamingStartedNotified = true;
        if (
            session.shouldTurnScreenOffOnStart &&
            session.controlSocket &&
            session.options.control === "true"
        ) {
            log(C.LogLevel.INFO, `[Session ${session.scid}] Sending initial screen off command.`);
            const powerMode = 0;
            const buffer = Buffer.alloc(2);
            buffer.writeUInt8(C.CONTROL_MSG_TYPE_SET_SCREEN_POWER_MODE, 0);
            buffer.writeUInt8(powerMode, 1);
            try {
                session.controlSocket.write(buffer);
                log(C.LogLevel.INFO, `[Session ${session.scid}] Sent initial screen off.`);
            } catch (e) {
                log(
                    C.LogLevel.ERROR,
                    `[Session ${session.scid}] Fail send initial screen off: ${e.message}`,
                );
                if (client.ws?.readyState === WebSocket.OPEN)
                    client.ws.send(
                        JSON.stringify({
                            type: C.MESSAGE_TYPES.ERROR,
                            message: `Fail send initial screen off: ${e.message}`,
                        }),
                    );
            }
        ...
        session.batteryInterval = setInterval(async () => {
            try {
                ...
                log(C.LogLevel.WARN, `Battery check failed for ${session.scid}: ${error.message}`);
            }
        }, 60000);
            ...
            try {
                if (await adbService.checkReverseTunnelExists(session.deviceId, tunnelString)) {
                    await adbService.executeCommand(
                        ...
                        `Remove tunnel post-connect (SCID: ${session.scid})`,
                    );
                }
                ...
                log(
                    C.LogLevel.WARN,
                    `Error removing tunnel post-connect for ${session.scid}: ${error.message}`,
                );
            }
        }
    }
...
/**
 * Attempts to identify the control socket by deduction if only one unidentified socket remains.
 */
function attemptIdentifyControlByDeduction(session, client) {
    ...
    if (
        session.controlSocket ||
        !isControlExpected ||
        session.socketsConnected < session.expectedSockets.length
    )
        return;
    ...
    if (videoIdentified && audioIdentified && unidentifiedCount === 1) {
        const [remainingSocketId, remainingSocket] = session.unidentifiedSockets
            .entries()
            .next().value;
        log(
            C.LogLevel.INFO,
            `[Session ${session.scid}] Identifying remaining socket ${remainingSocketId} as control.`,
        );
...
/**
 * Sets up a new scrcpy session, TCP server, and reverse tunnel for device streaming.
 * @param {string} deviceId - Device ID.
 * @param {string} scid - Session ID.
 * @param {number} port - TCP port.
 * @param {Object} runOptions - Streaming options.
 * @param {string} clientId - Client ID.
 * @param {string} displayMode - Display mode.
 * @param {boolean} shouldTurnScreenOffOnStartPref - Whether to turn screen off on start.
 * @param {Map} wsClientsRef - Reference to WebSocket clients.
 * @param {string} decoderType - Decoder type.
 * @returns {Promise<Object>} The created session object.
 */
async function setupScrcpySession(
    deviceId,
    scid,
    port,
    runOptions,
    clientId,
    displayMode,
    shouldTurnScreenOffOnStartPref,
    wsClientsRef,
    decoderType,
) {
    const session = {
    ...
    try {
        ...
        if (await adbService.checkReverseTunnelExists(deviceId, tunnelString)) {
            await adbService.executeCommand(
                `adb -s ${deviceId} reverse --remove ${tunnelString}`,
                `Remove specific tunnel (SCID: ${scid})`,
            );
        }
        await adbService
            .executeCommand(
        ...
        session.processStream.on("data", (data) =>
            log(C.LogLevel.INFO, `[scrcpy-server ${scid} std] ${data.toString().trim()}`),
        );
        session.processStream.on("error", (err) => {
            log(C.LogLevel.ERROR, `[scrcpy-server ${scid}] Stream error: ${err.message}`);
            cleanupSession(scid, wsClientsRef);
        });
        session.processStream.on("end", () => {
            log(C.LogLevel.INFO, `[scrcpy-server ${scid}] Stream ended.`);
        });
        return session;
    } catch (error) {
...
/**
 * Cleans up a scrcpy session, closes sockets, stops worker, and removes tunnels.
 * @param {string} scid - Session ID.
 * @param {Map} wsClientsRef - Reference to WebSocket clients.
 */
async function cleanupSession(scid, wsClientsRef) {
    ...
    if (processStream && typeof processStream.end === "function") {
        try {
            processStream.end();
            log(C.LogLevel.DEBUG, `[Cleanup] Ended process stream for ${scid}`);
        } catch (e) {
            log(C.LogLevel.WARN, `[Cleanup] Error ending process stream for ${scid}: ${e.message}`);
        }
    }
    if (tcpServer) {
        await new Promise((resolve) => tcpServer.close(resolve));
        log(C.LogLevel.DEBUG, `[Cleanup] Closed TCP server for ${scid}`);
    }
    const worker = workers.get(scid);
    if (worker) {
        worker.postMessage({ type: "stop" });
        workers.delete(scid);
        log(C.LogLevel.DEBUG, `[Cleanup] Stopped worker for ${scid}`);
    }
    if (session.tunnelActive && deviceId) {
        ...
        try {
            if (await adbService.checkReverseTunnelExists(deviceId, tunnelString)) {
                ...
                log(
                    C.LogLevel.INFO,
                    `[ADB] Removed reverse tunnel during cleanup: ${tunnelString}`,
                );
            }
        } catch (error) {
            log(
                C.LogLevel.WARN,
                `[ADB] Error removing reverse tunnel for ${scid}: ${error.message}`,
            );
        }
    }
